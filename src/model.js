...
function preprocess(canvas) {
  ...
  for (let i = 0; i < size * size; i++) {
    ...
    chw[size * size * 2 + i] = data[i * 4 + 2] / 255; // B
  }
...
export async function processImage(origCanvas) {
  ...
  const data = out.data; // Usually Float32Array with shape [1,1,h,w]
  const modelSize = MODEL_IO.inputSize;

  // Some exports yield [1,1,320,320], some flatten to 102400
  let src;
  if (data instanceof Float32Array && data.length === modelSize * modelSize) {
    ...
    if (h === modelSize && w === modelSize) {
      ...
      // If model returned different spatial size, upscale from that size
      const flat = new Float32Array(h * w);
      // Collapse from NCHW to flat HxW
    ...
    // Fallback: try to read first h*w slice
